...
def log_message(_time: time.struct_time, receiver: str, message: str, queue: str) -> None:
    """Logs the Message Information after it is Sent"""

    if not os.path.exists("WhatsAppMessageLog.txt"):
        file = open("WhatsAppMessageLog.txt", "w+")
        file.close()

    message = format_message(message)

    with open("WhatsAppMessageLog.txt", "a", encoding="utf-8") as file:
